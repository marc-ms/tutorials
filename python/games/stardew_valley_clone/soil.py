...
class Plant(pygame.sprite.Sprite):
    ...
    def grow(self):
        if self.check_watered(self.rect.center):
            ...
            # el jugador se mantiene detras de la planta cuando crece y colisiona
            if int(self.age) > 0:
                self.z = LAYERS['main']
...
class SoilLayer():
    ...
    def create_soil_grid(self):
        ...
        self.grid = [[[] for col in range(h_tiles)] for row in range(v_tiles)] ## minuto 3:55:10 creamos una serie de listas de listas que representan las celdas del suelo
        for x, y, _ in load_pygame('data/map.tmx').get_layer_by_name('Farmable').tiles(): # obtenemos las celdas donde se puede cultivar marcadas por una F
